...
const ParticipantsManager: React.FC<ParticipantsManagerProps> = ({ isSubscribed, aiUsage, refreshAiUsage, promptSubscription, isSidebarCollapsed }) => {
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 self-start md:self-auto">
            ...
            >
              ...
              <button onClick={() => setIsAddModalOpen(true)} className="bg-brand-accent-purple text-white dark:bg-brand-accent dark:text-brand-darker font-bold py-2 px-4 rounded-lg flex items-center justify-center space-x-2 hover:bg-opacity-90 transition">
                <PlusIcon />
                <span>Add New</span>
              </button>
          </div>
              ...
              {filteredParticipants.length > 0 ? (
                  ...
                  <tr key={p.id} className={`border-t border-light-border dark:border-brand-light/20 hover:bg-light-bg/70 dark:hover:bg-brand-light/20 transition-opacity duration-300 ${deletingId === p.id ? 'opacity-0' : ''}`}>
                      ...
                      <td className="p-4 flex justify-end space-x-2">
                        <button onClick={() => { setSelectedParticipant(p); setIsEditModalOpen(true); }} className="p-2 text-light-text-secondary hover:text-light-text dark:text-brand-text-secondary dark:hover:text-white transition"><EditIcon /></button>
                        <button onClick={() => { setSelectedParticipant(p); setIsDeleteModalOpen(true); }} className="p-2 text-light-text-secondary hover:text-red-500 dark:text-brand-text-secondary dark:hover:text-red-500 transition"><DeleteIcon /></button>
                      </td>
                  </tr>
                  ))
              ) : (
      ...
      {isAddModalOpen && <AddParticipantModal onClose={() => setIsAddModalOpen(false)} isSidebarCollapsed={isSidebarCollapsed} />}
      {isEditModalOpen && selectedParticipant && <EditParticipantModal participant={selectedParticipant} onClose={() => setIsEditModalOpen(false)} isSidebarCollapsed={isSidebarCollapsed} />}
      {isDeleteModalOpen && selectedParticipant && <DeleteParticipantModal participant={selectedParticipant} onClose={() => setIsDeleteModalOpen(false)} isSidebarCollapsed={isSidebarCollapsed} />}


      {isTutorialOpen && (
        <TutorialModal title="CSV Import Guide" onClose={() => setIsTutorialOpen(false)} isSidebarCollapsed={isSidebarCollapsed}>
          <div>
...
Charlie Chaplin,charlie@example.com,,Marketing,assist.charlie@example.com`}
      ...
      {isPaModalOpen && (
        <div className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center animate-fade-in px-8 py-4 ${isSidebarCollapsed ? 'lg:pl-[calc(5rem+2rem)]' : 'lg:pl-[calc(16rem+2rem)]'}`}>
            <div className="bg-light-surface dark:bg-brand-dark border border-light-border dark:border-brand-light/20 rounded-lg shadow-2xl w-full max-w-2xl max-w-full p-8 max-h-[90vh] flex flex-col">
